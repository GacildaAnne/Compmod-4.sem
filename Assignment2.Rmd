...
# Load data
Data <- read.csv("Ass2.csv", header = TRUE)

# Changing the column name for AltercentricIntrusion column to something easier
colnames(Data) [4] <- "AltInt"

# Take a quick look at the data
ggplot(Data, aes(AltInt, ID)) + geom_point()

```

```{r}

# Scaling the AltInt column (outcome) - because the original numbers/spread of data does not have any meaning, so scaled data may be convenient.
Data$scaledAlt <- scale(Data$AltInt)

# Take a look at the data before defining the prior
ggplot(Data, aes(scaledAlt)) + geom_density()


# Define the model - Altercentric intrusion is a function of the diagnosis
model0 <- bf(AltInt ~ 0 + Diagnosis)
model1 <- bf(scaledAlt ~ 0 + Diagnosis)

# Take a look at the data before defining the prior
ggplot(Data, aes(AltInt)) + geom_density()


# Define prior
get_prior(model1, Data, family = gaussian)

prior <-  prior(c(
  normal(0, 1), class = b,
  normal(1, 0.5), class = sigma)
  )
